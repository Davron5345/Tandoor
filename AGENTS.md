...
**Последнее обновление документации:** 2026-09-01 (себестоимость склада vs цена справочника)
...
| `/products` | Products.jsx | products.view; колонки **Цена справочника** и **Себест. склада** (филиал); в карточке — avg по отделам; клик по названию (при `documents.prihod` / `documents.view`) → модалка приходов; у варианта — только его строки (`variant_id`); у родителя — все по `product_id`; строка открывает `/prihod?open={id}` в **новой вкладке** |
