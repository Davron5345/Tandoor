...
**Последнее обновление документации:** 2026-09-19 (перемещение: по умолчанию между отделами)
...
| `/prihod`, `/rashod`, `/return-*`, `/transfer` | Documents.jsx | documents.*; фильтры: дата С/По, контрагент/поставщик, статус; `?open={id}` открывает документ; приход — вкладки **Товары** / **Доп. расходы** в том же окне (в себестоимость / в расходы; шапка и итоги всегда видны); в строке **Цена** стрелка ▲/▼ (дороже/дешевле vs последний приход; если цена как в последнем — vs предыдущий); в списке товара та же стрелка; перемещение — по умолчанию **между отделами**; компактная шапка в один ряд (Дата / Тип / Откуда / Куда), колонка «Нетто» + остаток склада «Откуда» под кол-вом и нетто; **телефон:** список карточками; окно документа (`.modal-doc-phone`) — жёлтая шапка ←/название/✓, дата+Итого сверху (`PhoneDateInput`), поля сеткой 2×2; товары как маркетплейс (фото + название + кол-во ±, sticky поиск → fullscreen sheet); **в перемещении цена/сумма/Итого скрыты**; «Провести» снизу |
