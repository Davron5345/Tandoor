...
**Последнее обновление документации:** 2026-07-28 (синхрон шапки банка с сайдбаром)
...
- Список дней: кнопка **«Столбцы»** — показать/скрыть колонки (portal поверх `.card`, чтобы меню не обрезалось; `localStorage` `bank_days_visible_columns_v1`); шапка колонок при скролле — **fixed pin** (`.bank-days-head-pin`), зелёная вспышка при закреплении; `left`/`width` синхронизируются с сайдбаром (ResizeObserver + rAF на смене `.app.sidebar-collapsed`)
