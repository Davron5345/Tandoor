...
**Последнее обновление документации:** 2026-09-01 (стрелка подорожало/подешевело у цен)
...
- **Округление:** `avg_cost` пишется с 4 знаками; кол-во в инвентаризации и отчёте остатков — 3 знака (без `24.400000000000002`)
...
| `/products` | Products.jsx | products.view; колонки **Цена справочника** и **Себест. склада** (филиал); у цен стрелка ▲/▼ по последним двум приходам (дороже/дешевле); в карточке — avg по отделам; клик по названию (при `documents.prihod` / `documents.view`) → модалка приходов; у варианта — только его строки (`variant_id`); у родителя — все по `product_id`; строка открывает `/prihod?open={id}` в **новой вкладке** |
...
| `/warehouse/prihod` | PrihodMobile.jsx | documents.prihod / documents.view (mobile приход) |
