...
**Последнее обновление документации:** 2026-08-28 (операционка: телефон/планшет минимум)
...
**Телефон (≤768px):** сайдбар — выезжающая панель поверх контента (иконка меню в `.main-topbar`); при открытии затемнение `.mobile-nav-backdrop`; при переходе на другую страницу меню закрывается. Состояние свёрнутости на телефоне в `localStorage` не пишется. Основной layout — как приложение: класс `app-phone-lock` на `html`/`body`/`#root` (из `App.jsx`, ≤768px; без кассы/MyShop) + CSS `position: fixed` / `height: 100%` / `min-height: 0` (не `100dvh`); `background-attachment: scroll`; скролл только внутри `.main-content` / списков (`overscroll-behavior: none`) — без «плавания» при жестах Safari. Касса (`app-cashier-mode`) на телефоне тоже фиксирует viewport. Операционка (документы, банк, номенклатура, контрагенты): фильтры 2 колонки, таблицы только в `.table-wrap`, модалки fullscreen с `footerPlacement="end"`, тач ≥44px; у номенклатуры/контрагентов скрыты второстепенные колонки.
...
| `/inventory` | Inventory.jsx | documents.inventory; документ по отделу: учёт / факт / разница; заполнение по учёту; проведение из карточки; **планшет** — компактная таблица (шапка 2 колонки), **телефон** — фиксированный экран как приложение, чипы статуса, FAB «Новый», карточки списка; карточка — шапка + поля сверху, кнопки в `modal-footer` (заголовок без `flex: 1` на колонку) |
