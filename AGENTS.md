...
**Последнее обновление документации:** 2026-09-01 (инвентаризация: не терять окно при обновлении)
...
| `/inventory` | Inventory.jsx | documents.inventory; **Частичная / Полная** кубиками с цветным фоном; отделы/сотрудник/отдел — те же кубики с палитрой; полная — статья списания + «В расход» / сотрудник / отдел; remainder связан с родителем; добавление строки берёт живой `book_qty`/`avg` отдела; **«+»** у выбора товара — `ProductCreateModal` как в приходе (`products.edit`); без вариантов сразу в документ, несколько вариантов — выбрать в списке; в строках **№** и **товар — вариант**; черновик **Сохранить** пишет факт/учёт/вариант как введены (окно не закрывается); черновик в `sessionStorage` — после обновления страницы окно восстанавливается; **Сохранить и провести**; проведение переснимает учёт; **карандаш** в списке (как НС): черновик открыть, проведённый — снять проведение → снова черновик; глаз — просмотр; список — **сумма остатков** (факт × складская средняя + списание непересчитанного у полной) и **Итого** под таблицей; в карточке полной — список **непересчитанных** позиций (клик по «Списание»); **телефон:** ☰ + заголовок + филиал + «Новый» в topbar; список карточек (№/статус/дата·отдел/сумма); **2 вкладки** документа; portal Modal |
