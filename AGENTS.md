...
**Последнее обновление документации:** 2026-07-27 (профиль сайдбара сверху)
...
**Футер сайдбара перенесён вверх (`sidebar-account` под логотипом):** филиал — иконка + select без подписи «Филиал»; профиль в одну строку (аватар, имя, иконка Telegram → `/telegram` с status title, выход). Плашка «Telegram не настроен» убрана.
...
- Список дней: кнопка **«Столбцы»** — показать/скрыть колонки (portal поверх `.card`, чтобы меню не обрезалось; `localStorage` `bank_days_visible_columns_v1`)
