...
**Последнее обновление документации:** 2026-09-01 (инвентаризация: кубики покрытия)
...
| `/inventory` | Inventory.jsx | documents.inventory; **Частичная / Полная** кубиками с цветным фоном; отделы/сотрудник/отдел — те же кубики с палитрой; полная — статья списания + «В расход» / сотрудник / отдел; remainder связан с родителем; добавление строки берёт живой `book_qty`/`avg` отдела; проведение переснимает учёт; **телефон:** ☰ + заголовок + филиал + «Новый» в topbar; список карточек (№/статус/дата·отдел/сумма); **2 вкладки** документа; portal Modal |
