...
**Последнее обновление документации:** 2026-09-19 (мобильный UI в стиле 1С)
...
**Телефон (≤768px):** сайдбар — выезжающая панель поверх контента (иконка меню в `.main-topbar`); при открытии затемнение `.mobile-nav-backdrop`; при переходе на другую страницу меню закрывается. Состояние свёрнутости на телефоне в `localStorage` не пишется. **Единый mobile shell (`app-mobile-admin`, стиль 1С):** палитра `--ones-*` (жёлтая шапка `#f5c518`, светлые поверхности, серые границы) принудительно светлая даже при тёмной теме desktop; заголовок страницы + домой/⋮; **dock** (Стол / Приход / Касса / Товары / Ещё) в flex-колонке `main` **скрыт на Главной**; скролл только в `.main-content > div`; при смене маршрута `scrollTop` сбрасывается; **Главная `/`:** сетка «Быстрый доступ» (2 кол. / 3 на ≥540px) + Telegram внизу (`.mobile-workspace-footer`); primary-кнопки жёлтые. Инвентаризация на телефоне: кнопка «Новый» — FAB над dock (и portal в шапку при наличии слота). Основной layout — как приложение: класс `app-phone-lock` на `html`/`body`/`#root` (из `App.jsx`, ≤768px; без MyShop) + CSS `overflow-x: hidden` / `touch-action: pan-y` / `height: 100%` / `min-height: 0`; скролл страницы только по вертикали (`.main-content > div`: `overflow-x: hidden`); горизонталь — только внутри `.table-wrap` / `.doc-modal-items-scroll` / `.section-tabs` (плюс JS-блок горизонтального pan вне этих зон). Операционка: фильтры 2 колонки, fullscreen-модалки с `footerPlacement="end"`, тач ≥44px; у номенклатуры/контрагентов скрыты второстепенные колонки.
