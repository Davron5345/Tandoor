...
**Последнее обновление документации:** 2026-08-28 (сайдбар: родители + вкладки подразделов)
...
Sidebar строится динамически по `hasPermission()`. В сайдбаре только **родительские** разделы (Закупки, Продажи, Справочники, Деньги, Производство, Отчёты, Администрирование) и плоские пункты (`Главная`, `Telegram`). Клик по родителю → переход на **первый доступный** child. Подразделы — горизонтальные вкладки (`.section-tabs`) в `<main>` сразу под `.main-topbar`; полоса вкладок только если у активной секции **≥ 2** пунктов. Аккордеон/flyout подразделов в сайдбаре нет.
