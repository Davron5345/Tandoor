...
**Последнее обновление документации:** 2026-08-27 (инвентаризация: телефон — колонка фильтров)
...
**Телефон (≤768px):** сайдбар — выезжающая панель поверх контента (кнопка «Меню» в `.main-topbar`); при открытии затемнение `.mobile-nav-backdrop`; при переходе на другую страницу меню закрывается. Состояние свёрнутости на телефоне в `localStorage` не пишется.
...
| `/inventory` | Inventory.jsx | documents.inventory; документ по отделу: учёт / факт / разница; заполнение по учёту; проведение из карточки; **планшет** — компактная таблица (шапка 2 колонки), **телефон** — список карточек, полноэкранная карточка документа, фильтры дат столбиком, кнопка «Новый» на всю ширину |
