...
**Последнее обновление документации:** 2026-07-27 (компактный футер сайдбара)
...
**Футер сайдбара (компактный):** филиал — иконка + select без подписи «Филиал»; профиль в одну строку (аватар, имя, иконка Telegram → `/telegram` с status title, выход). Плашка «Telegram не настроен» убрана.
