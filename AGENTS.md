...
**Последнее обновление документации:** 2026-08-20 (сумма и цена в строке документа)
...
**Избранное:** у подразделов (не у «Главная»/«Telegram» и не у самих групп) иконка звезды; клик делает её жёлтой и добавляет пункт в блок «Избранное» сверху (`localStorage` `warehouse-nav-favorites_v1`). Повторный клик снимает.
...
- **Нетто в приходе и перемещении** (`document_items.net_weight`): сумма строки = `qty × price` (цена за упаковку/шт); в UI цена и сумма двусторонние (ввод суммы → цена = сумма/кол-во, ввод цены → сумма = цена×кол-во). На склад идёт `stockQty = net × qty` при `net > 0`, иначе `qty`. В приходе `unitCost = amount / stockQty` (avg за ед. остатка, л/кг). Каталожное `products.net_weight` — только префилл строки в UI
