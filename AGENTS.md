...
**Последнее обновление документации:** 2026-08-28 (инвентаризация: phone-shell без резинки Safari)
...
| `/inventory` | Inventory.jsx | documents.inventory; документ по отделу: учёт / факт / разница; заполнение по учёту; проведение из карточки; **планшет** — компактная таблица (шапка 2 колонки), **телефон** — `inventory-phone-lock` + visualViewport (`--app-vvh`), один скролл списка/модалки с блокировкой rubber-band на краях, чипы статуса, FAB «Новый», карточки списка; карточка — шапка + поля сверху, кнопки в `modal-footer` |
