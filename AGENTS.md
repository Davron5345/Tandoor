...
**Последнее обновление документации:** 2026-07-27 (строгая изоляция филиалов = отдельные фирмы)
...
- Документы: тело запроса не может подменить `branch_id`; перемещение списывает **только** с активного филиала (`from_branch_id === req.branchId`)
