...
**Последнее обновление документации:** 2026-09-19 (mobile scroll audit)
...
**Телефон (≤768px):** сайдбар — выезжающая панель поверх контента (иконка меню в `.main-topbar`); при открытии затемнение `.mobile-nav-backdrop`; при переходе на другую страницу меню закрывается. Состояние свёрнутости на телефоне в `localStorage` не пишется. **Единый mobile shell (`app-mobile-admin`):** жёлтая шапка с названием страницы + домой/⋮; внизу dock (Стол / Приход / Касса / Товары / Ещё) в flex-колонке `main` (без двойного padding-bottom у контента); скролл только в `.main-content > div` (`-webkit-overflow-scrolling: touch`); при смене маршрута `scrollTop` сбрасывается; **Главная `/`:** сетка «Быстрый доступ» + Telegram. Инвентаризация на телефоне: кнопка «Новый» — FAB над dock (и portal в шапку при наличии слота). Основной layout — как приложение: класс `app-phone-lock` на `html`/`body`/`#root` (из `App.jsx`, ≤768px; без MyShop) + CSS `overflow-x: hidden` / `touch-action: pan-y` / `height: 100%` / `min-height: 0`; скролл страницы только по вертикали (`.main-content > div`: `overflow-x: hidden`); горизонталь — только внутри `.table-wrap` / `.doc-modal-items-scroll` / `.section-tabs` (плюс JS-блок горизонтального pan вне этих зон). Операционка: фильтры 2 колонки, fullscreen-модалки с `footerPlacement="end"`, тач ≥44px; у номенклатуры/контрагентов скрыты второстепенные колонки.
...
| Маршрут | Файл | Права |
|---------|------|-------|
| `/` | Dashboard.jsx | dashboard.view; «Топ по стоимости остатка» — `stock × avg_cost` склада; **телефон (≤768px):** «Рабочий стол» — сетка «Быстрый доступ» (`MobileWorkspaceHome` + `mobileQuickLinks.js`) вместо графиков; единая жёлтая шапка + нижний dock в `App.jsx` |
