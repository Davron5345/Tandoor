...
async function ensureBootstrap() {
  ...
  await execRaw("ALTER TABLE documents ADD COLUMN IF NOT EXISTS inventory_coverage TEXT DEFAULT 'partial'");
  await execRaw('ALTER TABLE documents ADD COLUMN IF NOT EXISTS article_id TEXT');
  await execRaw('ALTER TABLE documents ADD COLUMN IF NOT EXISTS liable_user_id TEXT');
  await execRaw('ALTER TABLE documents ADD COLUMN IF NOT EXISTS liable_department_id TEXT');
  await execRaw('ALTER TABLE payments ADD COLUMN IF NOT EXISTS liable_user_id TEXT');
  await execRaw('ALTER TABLE payments ADD COLUMN IF NOT EXISTS liable_department_id TEXT');
  ...
  try {
    await execRaw('ALTER TABLE payroll_employees ADD COLUMN IF NOT EXISTS view_token TEXT');
    await execRaw('CREATE UNIQUE INDEX IF NOT EXISTS idx_payroll_emp_view_token ON payroll_employees(view_token)');
  } catch (e) {
    console.error('⚠️ payroll view_token bootstrap:', e.message);
  }
  await execRaw(`
