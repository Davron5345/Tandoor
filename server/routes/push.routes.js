import { requireAnyPermission, attachBranch, requireAdmin } from '../middleware.js';
import {
  getVapidPublicKey,
  isPushEnabled,
...
/** Роли телефонного входа могут подписываться на push (не только снабжение). */
const PHONE_PUSH_PERMS = [
  'shop_orders.view',
  'cashier.view',
  'cashier.edit',
  'documents.prihod',
...
export function registerPushRoutes(app) {
  app.post('/api/push/subscribe', requireAnyPermission(...PHONE_PUSH_PERMS), attachBranch, (req, res) => {
    try {
      if (!isPushEnabled()) {
        return res.status(503).json({ error: 'Push-уведомления не настроены на сервере' });
      }
  ...
  app.post('/api/push/unsubscribe', requireAnyPermission(...PHONE_PUSH_PERMS), (req, res) => {
    try {
